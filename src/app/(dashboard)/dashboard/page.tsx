  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl flex-1 min-w-0"
      ...
      <div className="min-w-0">
        ...
        <p className="text-xl font-bold leading-tight" style={{ color: '#1e293b' }}>
          {value}<span className="text-sm font-normal ml-0.5" style={{ color: '#64748b' }}>{unit}</span>
        </p>
  ...
  return (
    <div onClick={() => router.push(href)}
      ...
      <div className="p-3">
        ...
        <p className="text-sm font-bold leading-tight" style={{ color: '#1e293b' }}>{judul}</p>
  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-white/5">
        ...
        <span className="flex-1 text-xs font-medium" style={{ color: '#64748b' }}>{sk.nama}</span>
      ...
      {open && (
        <div className="px-3 pb-3 space-y-2" style={{ background: 'rgba(0,0,0,0.1)' }}>
          <div className="grid grid-cols-2 gap-2 mt-2">
            {[
              { label: 'Produksi', val: `${sk.realisasiProduksi}/${sk.targetProduksi} ${sk.satuan}`, color: '#fbbf24' },
              ...
              { label: 'Bentuk Bantuan', val: `${sk.bentukIntervensi.length} jenis`, color: '#f59e0b' },
            ].map(item => (
            ...
            <div className="flex flex-wrap gap-1">
              {sk.bentukIntervensi.map(bi => (
                <span key={bi.id} className="text-xs px-2 py-0.5 rounded-full flex items-center gap-1"
                  ...
                  <Package size={9} style={{ color: '#fbbf24' }} /> {bi.nama}
                </span>
              ))}
            </div>
          )}
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-white/5 transition-colors">
        <Layers size={14} style={{ color: '#f59e0b', flexShrink: 0 }} />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold" style={{ color: '#64748b' }}>{k.nama}</p>
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(251,191,36,0.15)' }}>
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-white/5" onClick={() => setOpen(v => !v)}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold" style={{ color: '#1e293b' }}>{p.namaProgram}</p>
  ...
  return (
    <div className="relative w-full h-full rounded-xl overflow-hidden"
      ...
      <svg className="absolute inset-0 w-full h-full opacity-10" viewBox="0 0 100 100" preserveAspectRatio="none">
        {[20,40,60,80].map(x => <line key={`v${x}`} x1={x} y1="0" x2={x} y2="100" stroke="#fbbf24" strokeWidth="0.3" />)}
        {[20,40,60,80].map(y => <line key={`h${y}`} x1="0" y1={y} x2="100" y2={y} stroke="#fbbf24" strokeWidth="0.3" />)}
      </svg>
      {/* Kecamatan dots */}
      {kecamatanDots.map(dot => (
        <div key={dot.name} className="absolute flex flex-col items-center"
          style={{ left: `${dot.x}%`, top: `${dot.y}%`, transform: 'translate(-50%,-50%)' }}>
          <div className="w-2 h-2 rounded-full animate-pulse" style={{ background: '#fbbf24', boxShadow: '0 0 6px #fbbf24' }} />
          <span className="text-xs font-medium mt-0.5 whitespace-nowrap px-1 rounded"
            style={{ color: '#1e293b', background: 'rgba(0,0,0,0.5)', fontSize: 9 }}>
            {dot.name}
          </span>
        </div>
      ))}
      ...
      <div className="absolute top-2 left-3">
        <p className="text-xs font-bold uppercase tracking-wide" style={{ color: '#fbbf24' }}>Peta Sebaran Perikanan</p>
        <p className="text-xs" style={{ color: '#64748b' }}>Kabupaten Konawe</p>
      </div>
      {/* Button */}
...
export default function DashboardPage() {
  ...
  if (loading && !summary) return (
    <div className="flex flex-col items-center justify-center h-64 gap-4">
      <div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: '#fbbf24', borderTopColor: 'transparent' }} />
  ...
  const BIDANG = [
    { title: 'Tangkap', icon: Anchor, color: '#f59e0b', href: '/program', desc: 'Kelola data nelayan, kapal, alat tangkap, produksi tangkapan' },
    ...
    { title: 'Sekretariat', icon: Activity, color: '#64748b', href: '/program', desc: 'Kelola perencanaan, keuangan, kepegawaian, aset dan administrasi' },
    { title: 'Laporan & Analitik', icon: TrendingUp, color: '#fbbf24', href: '/monev', desc: 'Laporan, grafik, analisis dan unduh data perikanan' },
  ];
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold" style={{ color: '#1e293b' }}>Beranda — Dashboard Utama</h2>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1">
        <StatTopItem icon={Users} iconColor="#f59e0b" label="Nelayan / Penerima" value={fmtNum(summary?.totalPenerima ?? 0)} unit="Orang" trend={`${summary?.penerimaBulanIni ?? 0} bln ini`} />
        ...
        <StatTopItem icon={Activity} iconColor="#fbbf24" label="Total Kegiatan" value={summary?.totalKegiatan ?? 0} unit="Kegiatan" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-2 rounded-xl p-4 space-y-4"
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm font-bold" style={{ color: '#1e293b' }}>Ringkasan Kinerja Perikanan</p>
          </div>
          <div className="space-y-4">
            <KinerjaRow icon={Fish} iconBg="rgba(245,158,11,0.15)" label="Produksi Perikanan"
              value={avgProduksi}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        {/* Tren bulanan */}
        <div className="lg:col-span-2 rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(0,0,0,0.05)' }}>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold" style={{ color: '#1e293b' }}>Produksi Perikanan Bulanan (Ton)</p>
          </div>
          {tren.length > 0 ? (
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={tren} margin={{ top: 4, right: 8, left: -24, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(251,191,36,0.2)', borderRadius: 6, fontSize: 11 }} />
                <Line yAxisId="l" type="monotone" dataKey="penerima" stroke="#fbbf24" strokeWidth={2} dot={{ r: 3, fill: '#fbbf24' }} name="Penerima" />
        ...
        <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(0,0,0,0.05)' }}>
          <p className="text-sm font-bold mb-3" style={{ color: '#1e293b' }}>Sebaran per Kecamatan</p>
          {topSebaran.length > 0 ? (
            <div className="space-y-2.5">
              {topSebaran.map(([kec, jml], i) => (
                <div key={kec}>
                  <div className="flex justify-between text-xs mb-1">
                    <span style={{ color: '#64748b' }}>{i + 1}. {kec}</span>
                    <span style={{ color: '#fbbf24' }}>{jml}</span>
                  </div>
                  <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                    <div className="h-1.5 rounded-full" style={{ width: `${(jml / (topSebaran[0]?.[1] || 1)) * 100}%`, background: 'linear-gradient(90deg, #fbbf2488, #fbbf24)' }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(0,0,0,0.05)' }}>
          <p className="text-sm font-bold mb-3" style={{ color: '#1e293b' }}>Capaian per Program</p>
          {programs.length > 0 ? (
            <div className="space-y-2">
              {programs.slice(0, 4).map(p => (
                <div key={p.id} className="space-y-1">
                  <div className="flex justify-between text-xs">
                    <span className="truncate max-w-[120px]" style={{ color: '#64748b' }}>{p.namaProgram}</span>
      ...
      {/* ── Program Tree Detail ── */}
      {programs.length > 0 && (
        <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(251,191,36,0.1)' }}>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold" style={{ color: '#1e293b' }}>Detail Capaian Program → Kegiatan → Sub Kegiatan</p>
            <div className="flex items-center gap-3 text-xs" style={{ color: '#334155' }}>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full inline-block" style={{ background: '#fbbf24' }} /> Produksi</span>
